...
const TRANSFORMS: Transform[] = [
  ...
  },
  {
    ...
    flaggedForAI: true,
    replacement: (m) => `/* TODO: AI_REQUIRED — use getSignatureStatuses or waitForRecentTransactionConfirmation: ${m[1]}.getSignatureStatuses([${m[2].trim()}]).send() */`,
  },
  {
    category: "connection",
  ...
  // FLAGGED: sendAndConfirmTransaction(connection, tx, signers) — complex context
  {
    category: "transaction",
    pattern: /sendAndConfirmTransaction\s*\(\s*(\w+),\s*(\w+),\s*\[([^\]]+)\]\s*\)/g,
    flaggedForAI: true,
    replacement: (m) => `/* TODO: AI_REQUIRED — sendAndConfirmTransactionFactory({ rpc: ${m[1]} })(${m[2]}, { commitment: 'confirmed' }) */`,
  },
...
export const COVERAGE_CATEGORIES = [
  {
    name: "imports",
    description: "Rewrites all @solana/web3.js named imports to new split packages. Covers 50+ exports including sysvars, signers, rpc-types, accounts, and programs.",
    coveragePercent: 100,
    automated: true,
    transformCount: 50,
  },
  {
    name: "connection",
    description: "Migrates Connection to createSolanaRpc(). Adds .send() to all RPC calls. Covers getBalance, getAccountInfo, getProgramAccounts, requestAirdrop, getLatestBlockhash, and 8 more — 242 instances found.",
    coveragePercent: 92,
    automated: true,
    transformCount: 12,
  },
  {
    name: "keypair",
    description: "Keypair.generate() → await generateKeyPairSigner(). fromSecretKey/fromSeed → createKeyPairSignerFromBytes(). .publicKey → .address. 1,856 instances found.",
    coveragePercent: 90,
    automated: true,
    transformCount: 5,
  },
  {
    name: "publickey",
    description: "new PublicKey() → address(). findProgramAddressSync → getProgramDerivedAddress. Sysvar constants, equals(), toBase58(), isOnCurve(). 813 instances found.",
    coveragePercent: 95,
    automated: true,
    transformCount: 12,
  },
  {
    name: "transaction",
    description: "Auto-transforms: new Transaction(), .add(ix), .feePayer, .recentBlockhash, connection.sendTransaction(). Complex chained patterns and sendAndConfirmTransaction() are flagged with precise API guidance. 287 instances across tested repos.",
    coveragePercent: 55,
    automated: false,
    transformCount: 10,
  },
  {
    name: "buffer",
    description: "bs58.encode/decode → getBase58Encoder/Decoder codecs. Buffer.from/alloc/concat → Uint8Array. .toBuffer() → new Uint8Array(). 289 instances found.",
    coveragePercent: 100,
    automated: true,
    transformCount: 6,
  },
  {
    name: "lamports",
    description: "web3.LAMPORTS_PER_SOL → LAMPORTS_PER_SOL. Numeric SOL multiplication patterns → lamports(BigInt(...)). Fully automated.",
    coveragePercent: 100,
    automated: true,
    transformCount: 2,
  },
