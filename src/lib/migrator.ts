...
const TRANSFORMS: Transform[] = [
  ...
  {
    category: "connection",
    pattern: /(\w+)\.sendRawTransaction\s*\(\s*([^)]+)\s*\)/g,
    flaggedForAI: true,
    replacement: (m) => `/* TODO: AI_REQUIRED — sendTransactionFactory({ rpc: ${m[1]} })(${m[2].trim()}) — use getBase64EncodedWireTransaction() */`,
  },
  // getBlockTime — auto
  {
    category: "connection",
  ...
  {
    category: "connection",
    pattern: /(\w+)\.getNonce\s*\(\s*([^)]+)\s*\)/g,
    flaggedForAI: true,
    replacement: (m) => `/* TODO: AI_REQUIRED — use durable nonce via getAccountInfo + NonceAccount.fromAccountData in @solana/kit */`,
  },
  // web3.Connection / web3.Keypair / web3.PublicKey namespace forms
  {
    category: "connection",
    pattern: /web3\.Connection\b/g,
    replacement: "createSolanaRpc",
  },
  {
    category: "keypair",
    pattern: /web3\.Keypair\b/g,
    replacement: "/* generateKeyPairSigner */ await generateKeyPairSigner",
  },
  {
    category: "publickey",
    pattern: /web3\.PublicKey\b/g,
    replacement: "address",
  },
  ...
  // PublicKey.default → the system program address (all zeros)
  {
    category: "publickey",
    pattern: /PublicKey\.default\b/g,
    replacement: "address('11111111111111111111111111111111')",
  },
  {
    category: "publickey",
...
export const COVERAGE_CATEGORIES = [
  ...
  {
    name: "connection",
    description: "Migrates Connection to createSolanaRpc(). Adds .send() to all RPC calls. Covers 27 methods: getBalance, getAccountInfo, getMinimumBalanceForRentExemption, getRecentBlockhash→getLatestBlockhash, getVersion, getSignatureStatuses, simulateTransaction, getBlockTime, getEpochInfo, getRecentPerformanceSamples, getTokenAccountsByOwner, requestAirdrop, and more. 432 instances found.",
    coveragePercent: 94,
    automated: true,
    transformCount: 27,
  },
...
export const REPO_TEST_RESULTS = [
  ...
  },
  {
    ...
    note: "The SDK's own test fixtures — high automation with one confirmTransaction pattern requiring manual review.",
  },
  {
  ...
  },
  {
    ...
    note: "Largest sample repo — high automation across 146 files. 381 transaction transforms, most now fully automated.",
  },
];
...
// Comprehensive pattern reference — every rule the engine supports with before/after
export const PATTERN_REFERENCE: Array<{
  category: string;
  name: string;
  auto: boolean;
  before: string;
  ...
  // ── Imports ────────────────────────────────────────────────────────────────
  { category: "imports", name: "Named import rewrite", auto: true,
    before: `import { Connection, Keypair, PublicKey } from '@solana/web3.js'`,
    after:  `import { createSolanaRpc } from '@solana/rpc'\nimport { generateKeyPairSigner } from '@solana/signers'\nimport { address } from '@solana/addresses'` },
  { category: "imports", name: "LAMPORTS_PER_SOL import", auto: true,
    before: `import { LAMPORTS_PER_SOL } from '@solana/web3.js'`,
    after:  `import { LAMPORTS_PER_SOL } from '@solana/kit'` },
  { category: "imports", name: "Sysvar imports", auto: true,
    before: `import { SYSVAR_RENT_PUBKEY, SYSVAR_CLOCK_PUBKEY } from '@solana/web3.js'`,
    after:  `import { SYSVAR_RENT_ADDRESS, SYSVAR_CLOCK_ADDRESS } from '@solana/sysvars'` },
  { category: "imports", name: "Transaction imports", auto: true,
    before: `import { Transaction, sendAndConfirmTransaction } from '@solana/web3.js'`,
    after:  `import { Transaction, sendAndConfirmTransaction } from '@solana/transactions'` },

  // ── Connection ────────────────────────────────────────────────────────────
  { category: "connection", name: "new Connection()", auto: true,
    before: `new Connection('https://api.mainnet-beta.solana.com')`,
    after:  `createSolanaRpc('https://api.mainnet-beta.solana.com')` },
  { category: "connection", name: "getBalance", auto: true,
    before: `await connection.getBalance(publicKey)`,
    after:  `await connection.getBalance(publicKey).send()` },
  { category: "connection", name: "getAccountInfo", auto: true,
    before: `await connection.getAccountInfo(address)`,
    after:  `await connection.getAccountInfo(address).send()` },
  { category: "connection", name: "getLatestBlockhash", auto: true,
    before: `await connection.getLatestBlockhash()`,
    after:  `await connection.getLatestBlockhash().send()` },
  { category: "connection", name: "getRecentBlockhash (deprecated)", auto: true,
    before: `await connection.getRecentBlockhash()`,
    after:  `await connection.getLatestBlockhash().send() /* NOTE: getRecentBlockhash deprecated → getLatestBlockhash */` },
  { category: "connection", name: "getSlot", auto: true,
    before: `await connection.getSlot()`,
    after:  `await connection.getSlot().send()` },
  { category: "connection", name: "getProgramAccounts", auto: true,
    before: `await connection.getProgramAccounts(programId)`,
    after:  `await connection.getProgramAccounts(programId).send()` },
  { category: "connection", name: "getMultipleAccountsInfo", auto: true,
    before: `await connection.getMultipleAccountsInfo([pk1, pk2])`,
    after:  `await connection.getMultipleAccounts([pk1, pk2]).send()` },
  { category: "connection", name: "getTokenAccountBalance", auto: true,
    before: `await connection.getTokenAccountBalance(ata)`,
    after:  `await connection.getTokenAccountBalance(ata).send()` },
  { category: "connection", name: "getTokenAccountsByOwner", auto: true,
    before: `await connection.getTokenAccountsByOwner(owner, filter)`,
    after:  `await connection.getTokenAccountsByOwner(owner, filter).send()` },
  { category: "connection", name: "getMinimumBalanceForRentExemption", auto: true,
    before: `await connection.getMinimumBalanceForRentExemption(165)`,
    after:  `await connection.getMinimumBalanceForRentExemption(165).send()` },
  { category: "connection", name: "getVersion", auto: true,
    before: `await connection.getVersion()`,
    after:  `await connection.getVersion().send()` },
  { category: "connection", name: "getSignatureStatuses", auto: true,
    before: `await connection.getSignatureStatuses([sig])`,
    after:  `await connection.getSignatureStatuses([sig]).send()` },
  { category: "connection", name: "simulateTransaction", auto: true,
    before: `await connection.simulateTransaction(tx)`,
    after:  `await connection.simulateTransaction(tx).send()` },
  { category: "connection", name: "getBlockTime", auto: true,
    before: `await connection.getBlockTime(slot)`,
    after:  `await connection.getBlockTime(slot).send()` },
  { category: "connection", name: "getEpochInfo", auto: true,
    before: `await connection.getEpochInfo()`,
    after:  `await connection.getEpochInfo().send()` },
  { category: "connection", name: "getRecentPerformanceSamples", auto: true,
    before: `await connection.getRecentPerformanceSamples(5)`,
    after:  `await connection.getRecentPerformanceSamples(5).send()` },
  { category: "connection", name: "requestAirdrop", auto: true,
    before: `await connection.requestAirdrop(publicKey, 1e9)`,
    after:  `await connection.requestAirdrop(publicKey, 1e9).send()` },
  { category: "connection", name: "getTransaction", auto: true,
    before: `await connection.getTransaction(sig)`,
    after:  `await connection.getTransaction(sig).send()` },
  { category: "connection", name: "clusterApiUrl", auto: true,
    before: `clusterApiUrl('mainnet-beta')`,
    after:  "`https://api.mainnet-beta.solana.com`" },
  { category: "connection", name: "sendRawTransaction ⚠", auto: false,
    before: `await connection.sendRawTransaction(tx.serialize())`,
    after:  `/* TODO: AI_REQUIRED — sendTransactionFactory({ rpc: connection })(tx) — use getBase64EncodedWireTransaction() */` },
  { category: "connection", name: "confirmTransaction ⚠", auto: false,
    before: `await connection.confirmTransaction(sig)`,
    after:  `/* TODO: AI_REQUIRED — await connection.getSignatureStatuses([sig]).send() then check confirmations */` },
  { category: "connection", name: "web3.Connection namespace", auto: true,
    before: `web3.Connection`,
    after:  `createSolanaRpc` },

  // ── Keypair ───────────────────────────────────────────────────────────────
  { category: "keypair", name: "Keypair.generate()", auto: true,
    before: `Keypair.generate()`,
    after:  `await generateKeyPairSigner()` },
  { category: "keypair", name: "Keypair.fromSecretKey()", auto: true,
    before: `Keypair.fromSecretKey(secretKeyBytes)`,
    after:  `await createKeyPairSignerFromBytes(secretKeyBytes)` },
  { category: "keypair", name: "Keypair.fromSeed()", auto: true,
    before: `Keypair.fromSeed(seed)`,
    after:  `await createKeyPairSignerFromBytes(seed)` },
  { category: "keypair", name: ".publicKey → .address", auto: true,
    before: `keypair.publicKey`,
    after:  `keypair.address` },
  { category: "keypair", name: "web3.Keypair namespace", auto: true,
    before: `web3.Keypair.generate()`,
    after:  `await generateKeyPairSigner()` },
  { category: "keypair", name: ".secretKey ⚠", auto: false,
    before: `keypair.secretKey`,
    after:  `keypair.secretKey /* TODO: AI_REQUIRED — secretKey not exposed in @solana/kit KeyPairSigner */` },

  // ── PublicKey ─────────────────────────────────────────────────────────────
  { category: "publickey", name: "new PublicKey()", auto: true,
    before: `new PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v')`,
    after:  `address('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v')` },
  { category: "publickey", name: "findProgramAddressSync", auto: true,
    before: `PublicKey.findProgramAddressSync([Buffer.from('seed')], programId)`,
    after:  `await getProgramDerivedAddress({ programAddress: programId, seeds: [new Uint8Array('seed')] })` },
  { category: "publickey", name: "findProgramAddress (async)", auto: true,
    before: `await PublicKey.findProgramAddress([Buffer.from('seed')], programId)`,
    after:  `await getProgramDerivedAddress({ programAddress: programId, seeds: [new Uint8Array('seed')] })` },
  { category: "publickey", name: "PublicKey.isOnCurve()", auto: true,
    before: `PublicKey.isOnCurve(bytes)`,
    after:  `isAddress(bytes)` },
  { category: "publickey", name: ".toBase58()", auto: true,
    before: `publicKey.toBase58()`,
    after:  `publicKey` },
  { category: "publickey", name: ".equals()", auto: true,
    before: `pk1.equals(pk2)`,
    after:  `(pk1 === pk2)` },
  { category: "publickey", name: "PublicKey.default", auto: true,
    before: `PublicKey.default`,
    after:  `address('11111111111111111111111111111111')` },
  { category: "publickey", name: "SystemProgram.programId", auto: true,
    before: `SystemProgram.programId`,
    after:  `SYSTEM_PROGRAM_ADDRESS` },
  { category: "publickey", name: "TOKEN_PROGRAM_ID", auto: true,
    before: `TOKEN_PROGRAM_ID`,
    after:  `TOKEN_PROGRAM_ADDRESS` },
  { category: "publickey", name: "Sysvar addresses", auto: true,
    before: `SYSVAR_RENT_PUBKEY\nSYSVAR_CLOCK_PUBKEY`,
    after:  `SYSVAR_RENT_ADDRESS\nSYSVAR_CLOCK_ADDRESS` },
  { category: "publickey", name: "PublicKey.createWithSeed ⚠", auto: false,
    before: `PublicKey.createWithSeed(base, seed, programId)`,
    after:  `/* TODO: AI_REQUIRED — getAddressWithSeed({ baseAddress, programAddress, seed }) */` },

  // ── Transaction ───────────────────────────────────────────────────────────
  { category: "transaction", name: "new Transaction()", auto: true,
    before: `new Transaction()`,
    after:  `createTransactionMessage({ version: 0 })` },
  { category: "transaction", name: "tx.add(ix)", auto: true,
    before: `tx.add(instruction)`,
    after:  `appendTransactionMessageInstruction(instruction, tx)` },
  { category: "transaction", name: "tx.feePayer = signer", auto: true,
    before: `tx.feePayer = payer.publicKey`,
    after:  `tx = setTransactionMessageFeePayerSigner(payer.address, tx)` },
  { category: "transaction", name: "tx.recentBlockhash = hash", auto: true,
    before: `tx.recentBlockhash = blockhash`,
    after:  `tx = setTransactionMessageLifetimeUsingBlockhash({ blockhash, lastValidBlockHeight: BigInt(0) }, tx)` },
  { category: "transaction", name: "sendAndConfirmTransaction (named tx)", auto: true,
    before: `await sendAndConfirmTransaction(connection, tx, [payer])`,
    after:  `await sendAndConfirmTransactionFactory({ rpc: connection })(tx, { commitment: 'confirmed' })` },
  { category: "transaction", name: "connection.sendTransaction()", auto: true,
    before: `await connection.sendTransaction(tx, [payer])`,
    after:  `await sendAndConfirmTransactionFactory({ rpc: connection })(tx, { commitment: 'confirmed' })` },
  { category: "transaction", name: "new Transaction().add() chained ⚠", auto: false,
    before: `new Transaction().add(instruction1).add(instruction2)`,
    after:  `/* TODO: AI_REQUIRED — pipe(createTransactionMessage({version:0}), appendTransactionMessageInstructions([instruction1, instruction2])) */` },
  { category: "transaction", name: "new VersionedTransaction() ⚠", auto: false,
    before: `new VersionedTransaction(message)`,
    after:  `/* TODO: AI_REQUIRED — compileTransaction(message) */` },

  // ── Buffer ────────────────────────────────────────────────────────────────
  { category: "buffer", name: "Buffer.from()", auto: true,
    before: `Buffer.from(data)`,
    after:  `new Uint8Array(data)` },
  { category: "buffer", name: "Buffer.alloc()", auto: true,
    before: `Buffer.alloc(32)`,
    after:  `new Uint8Array(32)` },
  { category: "buffer", name: "Buffer.concat()", auto: true,
    before: `Buffer.concat([a, b])`,
    after:  `new Uint8Array([...[a, b].flat()])` },
  { category: "buffer", name: ".toBuffer()", auto: true,
    before: `publicKey.toBuffer()`,
    after:  `new Uint8Array(publicKey)` },
  { category: "buffer", name: "bs58.encode()", auto: true,
    before: `bs58.encode(bytes)`,
    after:  `getBase58Encoder().encode(bytes)` },
  { category: "buffer", name: "bs58.decode()", auto: true,
    before: `bs58.decode(str)`,
    after:  `getBase58Decoder().decode(str)` },

  // ── Lamports ──────────────────────────────────────────────────────────────
  { category: "lamports", name: "web3.LAMPORTS_PER_SOL", auto: true,
    before: `web3.LAMPORTS_PER_SOL`,
    after:  `LAMPORTS_PER_SOL` },
  { category: "lamports", name: "SOL multiplication", auto: true,
    before: `1.5 * LAMPORTS_PER_SOL`,
    after:  `lamports(BigInt(1.5 * 1_000_000_000))` },
];
